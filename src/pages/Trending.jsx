...
import { TrendingContext } from "../ContextApi/TrendingContext";

const Trending = () => {
  const { trendingData, resetTrendingCoins } = useContext(TrendingContext);

  return (
    <div className="w-[80%] mt-[4rem] flex flex-col first:items-end">
      <button className="mb-4 ml-auto" onClick={resetTrendingCoins}>
        <svg
      ...
      <div className="w-full border border-gray-100 rounded min-h-[60vh] px-[20px] md:px-10 lg:px-0  py-10 flex flex-wrap justify-evenly gap-[60px] lg:gap-10">
        {trendingData ? (
          trendingData.map((data) => (
            <TrendingBox key={data.item.id} item={data.item} />
          ))
        ) : (
